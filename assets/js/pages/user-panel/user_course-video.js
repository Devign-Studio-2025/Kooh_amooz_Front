...
class CommentFormStateManager {
  constructor() {
    ...
    this.requiredFields = ['input[name="rating"]'];
  ...
  updateStarDisplay(rating = 0) {
    ...
    document
      .querySelectorAll(".star-label .comments-score__icon svg use")
      .forEach((use) => {
    ...
    for (let i = 1; i <= rating; i++) {
      ...
      if (starInput) {
        const useElement = starInput.nextElementSibling.querySelector(
          ".comments-score__icon svg use"
        );
        useElement.setAttribute(
          "href",
          "assets/icons/sprites.svg#iconoir:star-solid"
        );
      }
    }
  }
  ...
  isFormValid() {
    ...
    const commentValid =
      commentText === "" ||
      (commentText.length >= 10 && commentText.length <= 500);
  ...
  updateCommentsStatistics() {
    ...
    comments.forEach((comment) => {
      const ratingElement = comment.querySelector(
        ".comment__score .comments-score__count"
      );
    ...
    document.getElementById(
      "total-ratings"
    ).textContent = `از مجموع ${totalRatings} امتیاز`;
    document.getElementById(
      "total-comments"
    ).textContent = `${commentCount} نظر`;
  }
  ...
  resetForm() {
    ...
    // Re-init char limit
    initCharLimit("#comment-text", 500, 500);
